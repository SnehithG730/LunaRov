...
export class DStarLitePathfinder implements IPathfinder {
  public findPath(
    ...
    if (start.x === target.x && start.y === target.y) {
      const cell = grid.cells[start.y]?.[start.x];
      return {
    ...
    if (
      ...
      return {
        ...
        computeTimeMs: Number((performance.now() - startTime).toFixed(2)),
        success: false,
        failureReason: 'Start or destination point is out of sector grid bounds.',
        estimatedEnergyWh: 0,
      };
    }
    ...
    if (isCellBlocked(startCell, options)) {
      return {
        ...
        computeTimeMs: Number((performance.now() - startTime).toFixed(2)),
        success: false,
        failureReason: `Rover start position [${start.x}, ${start.y}] is located on an impassable obstacle or cliff.`,
        estimatedEnergyWh: 0,
      };
    }
    ...
    if (isCellBlocked(targetCell, options)) {
      return {
        ...
        computeTimeMs: Number((performance.now() - startTime).toFixed(2)),
        success: false,
        failureReason: `Mission destination target [${target.x}, ${target.y}] is located on an impassable hazard.`,
        estimatedEnergyWh: 0,
      };
    }
    ...
    if (rawPath.length === 0) {
      return {
        ...
        success: false,
        failureReason: 'No traversable path exists between start and destination (terrain impassable).',
        estimatedEnergyWh: 0,
      };
    }
    ...
    // Calculate total movement cost and 3D ground distance
    ...
    const startDNode = engine.getNode(start.x, start.y);
    const estimatedEnergyWh = Number(((startDNode?.g !== Infinity ? (startDNode?.g ?? 0) * 0.45 : totalMovementCost * 0.45)).toFixed(1));
